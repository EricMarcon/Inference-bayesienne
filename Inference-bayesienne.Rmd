...
La vraisemblance de $\theta|\mathbf{Y}$ est proportionnelle au produit des vraisemblances de $\mathbf{Y}|\theta$ et de la valeur _a priori_ (c'est-à-dire avant de connaître les données) de $\theta$.
Chacune des deux vraisemblances doit être écrite en foncton de $\theta$.
...
La fonction de propostion permet d'explorer l'espace des paramètres, c'est-à-dire toutes les valeurs possibles de $\theta$.
...
Le nombre d'itération doit être grand.
